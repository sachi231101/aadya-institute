...
import { findUserById, findUsersByEmailOrPhone } from "./auth.repository";
import {
  createRefreshToken,
  findRefreshToken,
  revokeRefreshToken,
...
export const loginService = async (
  ...
  const candidates = await findUsersByEmailOrPhone(input.emailOrPhone);

  // Always return the same generic error to prevent user enumeration
  if (candidates.length === 0) throw new AppError("Invalid credentials", 401);

  // When duplicate emails exist, authenticate against the account whose password matches.
  let user = candidates[0];
  let passwordMatched = false;
  for (const candidate of candidates) {
    if (candidate.status !== "ACTIVE") continue;
    const ok = await comparePassword(input.password, candidate.passwordHash);
    if (ok) {
      user = candidate;
  ...
  if (!passwordMatched) {
    user = candidates.find((c) => c.status === "ACTIVE") ?? candidates[0];
  }
