...
async function main() {
  console.log("🧹 Starting Complete Database Wipe (Keeping Only Admin Login Data)...");

  // 1. Fetch all public tables except _prisma_migrations
  ...
  for (const { tablename } of tablenames) {
    try {
      ...
      console.warn(`  ⚠️ Failed to truncate ${tablename}: ${err.message}`);
    }
  }

  console.log("✅ All operational tables truncated successfully.");

  // 3. Re-seed base setup and Admin user
  console.log("\n🌱 Seeding Base System & Admin Account...");
  ...
  // B. Branches
  const branchKormangala = await prisma.branch.create({
    data: {
  ...
  console.log("✓ Created 3 Branches: KOR, IND, HSR");
  ...
  console.log("✓ Created 5 Base Roles");
  ...
  const facultyPermNames = [
    ...
    "google_meet.create", "google_meet.read", "google_meet.manage", "chat.read",
    "chat.send", "feedback.read", "notification.read", "report.read",
    "exam.read", "question.read", "question_bank.read",
  ];
  ...
  const studentPermNames = [
    "dashboard.read", "course.read", "module.read", "batch.read", "schedule.read",
    "attendance.read", "assignment.read", "assignment.submit", "recording.read",
    "google_meet.read", "feedback.create", "notification.read", "exam.read", "exam.take",
  ];
  ...
  console.log("✓ Assigned Role Permissions to all 5 roles");

  // E. ADMIN USER
  console.log("\n👤 Creating Sole Admin User...");
  ...
  // Grant all permissions directly in UserPermission for Admin
  for (const perm of Object.values(permissions)) {
  ...
  console.log(`✓ Admin user created: ${adminUser.email} with ADMIN role & ${permissionsList.length} direct permissions`);
  ...
  console.log("\n===================================================================");
  console.log("🎉 DATABASE RESET COMPLETE: ONLY ADMIN DATA EXISTS");
  console.log("===================================================================");
