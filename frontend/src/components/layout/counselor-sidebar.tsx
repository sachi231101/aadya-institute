...
import {
  ...
  buildCounselorNavPermissionKeys,
} from "@/constants/counselor-portal-nav";
import {
  canAccessNavUrl,
...
export function CounselorSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const filteredNavItems = React.useMemo(() => {
    ...
    return COUNSELOR_PORTAL_NAV.map((item) => {
    ...
    <Sidebar collapsible="icon" {...props} className="border-r border-border/50 bg-bg-secondary">
      <SidebarHeader className="h-12 border-b border-[#334155] bg-[#172033] text-white px-3 flex flex-row items-center">
        ...
        >
          ...
          />
          <span className="text-[10px] text-emerald-300 font-bold tracking-wider bg-white/10 border border-white/20 px-2 py-0.5 rounded-full shrink-0 group-data-[collapsible=icon]:hidden">
            COUNSELLOR
          </span>
        </Link>
      </SidebarHeader>
                  ...
                  return (
                    <SidebarMenuItem key={item.title} className="mb-2">
                      <SidebarMenuButton
                        ...
                        tooltip={item.title}
                        className="bg-gradient-to-r from-emerald-600/10 to-teal-500/10 border border-emerald-600/20 hover:from-emerald-600/15 hover:to-teal-500/15"
                      >
                        <Link to={item.url} className="flex items-center gap-2.5 w-full">
                          <item.icon className="h-4 w-4 shrink-0 text-emerald-700" />
                          <span className="truncate font-semibold text-emerald-700">{item.title}</span>
                        </Link>
                      </SidebarMenuButton>
                    </SidebarMenuItem>
                  );
                ...
                return (
                  <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton asChild isActive={isItemActive} tooltip={item.title}>
                      <Link to={item.url} className="flex items-center gap-2.5 w-full">
                        <item.icon
                          className={`h-4 w-4 shrink-0 ${isItemActive ? "text-emerald-700 font-semibold" : "text-muted-foreground"}`}
                        />
                    ...
                    <CollapsibleTrigger asChild>
                      <SidebarMenuButton tooltip={item.title} isActive={isGroupActive} className="w-full justify-between">
                        <div className="flex items-center gap-2.5 min-w-0 flex-1">
                          <item.icon
                            className={`h-4 w-4 shrink-0 ${isGroupActive ? "text-emerald-700 font-semibold" : "text-muted-foreground"}`}
                          />
                          <span className="truncate min-w-0 flex-1 text-[13.5px] font-medium">{item.title}</span>
                        </div>
      ...
      <SidebarFooter className="border-t border-border/50 p-2 space-y-2">
        ...
        <div className="p-2.5 rounded-xl bg-emerald-50/70 border border-emerald-200/60 flex items-center justify-between">
