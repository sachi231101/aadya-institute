...
import { Loader2, AlertCircle, Inbox, Eye, Download } from "lucide-react";
...
function queueQuery(filter: QueueFilter) {
  if (filter === "NEEDS_REVIEW") return { ungradedOnly: true as const };
  if (filter === "GRADED") return { status: "GRADED" as const };
  if (filter === "LATE") return { status: "LATE" as const };
  return { submittedOnly: true as const };
}
...
export const SubmissionsQueue: React.FC = () => {
  ...
  const query = useAssignmentSubmissions({
    page,
    limit: 50,
    ...queueQuery(statusFilter),
    ...(search.trim() ? { search: search.trim() } : {}),
  });

  const submissions = (query.data?.data || []) as AssignmentSubmission[];
    ...
    <div className="w-full p-4 sm:p-6 lg:p-8 space-y-6 pb-16">
        ...
        <CardContent className="p-4 space-y-4">
          <div className="flex flex-col md:flex-row gap-3">
            ...
            <select
              value={statusFilter}
              onChange={(e) => {
                setStatusFilter(e.target.value as QueueFilter);
                setPage(1);
              }}
                    ...
                    >
                      ...
                      <TableCell className="text-right space-x-1 px-3.5 py-3">
                        <Button
                          ...
                          onClick={() =>
                            navigate(
                              `${basePath}/assignments/${submission.assignment?.id || submission.assignmentId}`
                            )
                          }
                        >
      ...
      <Dialog open={!!grading} onOpenChange={(open) => !open && setGrading(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Grade — {grading?.student?.user?.name || "Student"} ({grading?.assignment?.title})
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            <p className="text-text-secondary">Due: {formatAssignmentDueDate(grading?.assignment?.dueDate)}</p>
